...
import { Selection } from "azure-devops-ui/Utilities/Selection";
...
import {tasks} from "./StoryStepData"
...
export class QuickStepAdmin extends React.Component<{}, MyAdminStates>{
  ...
  private newStepTitle = new ObservableValue<string>("");
  ...
  public onClickRemove() {
    tasks.splice(this.selection.value[0].beginIndex, 1);
    let arrayItemProvider = new ArrayItemProvider(tasks);
    this.setState({
  ...
  public determineMoveDownState() {
    if (this.selection.value[0].beginIndex + 1 >= tasks.length) {
      this.setState({
        moveDownStepDisabled: true
        // StoryRecordsArray: storiesplaceholder
      });
  ...
  public async onClickMoveDown() {
    ...
    [
      tasks[this.selection.value[0].beginIndex],
      tasks[this.selection.value[0].beginIndex + 1]
    ] = [
      tasks[this.selection.value[0].beginIndex + 1],
      tasks[this.selection.value[0].beginIndex]
    ];
...
export default QuickStepAdmin;



// showRootComponent(<QuickStepsUser/>);
showRootComponent(<QuickStepAdmin/>);
